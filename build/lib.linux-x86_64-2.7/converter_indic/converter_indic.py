...
This module is a UTF (Indian Scripts) to Roman (WX) convertor and vice-versa that:
    ...
    - converts text in 9 Indian languages viz. Hindi, Tamil, Telegu, Malayalam, 
      Bengali, Kannada, Oriya, Punjabi and Marathi.
...
class wxconvert():
    ...
    and Marathi in 5 data formats viz. plain-text, ssf, conll, bio and tnt.
    """

    def __init__(self, src_trg, op_format="text", lang="hin"):
	self.lang = lang
	self.op_format = op_format
	wxp = wxilp(self.lang, src_trg)
	self.transform = wxp.wx2utf if src_trg=="wx2utf" else wxp.utf2wx
	...
	if self.op_format=="text":
	    ...
	    print >>sys.stderr, "FormatError: invalid format :: %s" %self.op_format
	    sys.exit(0)
