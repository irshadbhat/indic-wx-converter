...
This is an Object Oriented implementation of convertor-indic tool developed by Rashid 
Ahmad, written in Perl. This module is a direct coversion of 'IndicCC.pl' script from
convertor-indic tool.

NOTE: This class is working exactly as IndicCC.pl works. I have tested it against
      implementation from Rashid Ahmad to ensure that the same output is generated 
      by both scripts. The execution time of both the scripts for any input is almost 
      equal but it can be reduced (for python implementation) by using compiled regexes
      and will do it soon. 
"""
...
class wxilp():
	...
	q, l, n, r, a = u'q' in my_string, u'l' in my_string, u'n' in my_string, u'r' in my_string, u'a' in my_string
        ...
        if eV:
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    	   		([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])eV([MHz])''', lambda m: self.hashc_w2i[m.group(1)]+
    	   		self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+
			self.hashm_w2i[u"aeV"]+self.hashmd_w2i[m.group(4)], my_string)
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])eV''', lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+
    			self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+self.hashm_w2i[u"aeV"], my_string)
        if EY:
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])EY([MHz])''',lambda m: self.hashc_w2i[m.group(1)]+
			self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+
			self.hashm_w2i[u"aEY"]+self.hashmd_w2i[m.group(4)], my_string)
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])EY''',lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+
    			self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+self.hashm_w2i[u"aEY"], my_string)
        if oV:
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])oV([MHz])''', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+
			self.hashm_w2i[u"aoV"]+self.hashmd_w2i[m.group(4)], my_string)
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])oV''', lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+
    			self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+self.hashm_w2i[u"aoV"], my_string)
        if OY:
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])OY([MHz])''',lambda m: self.hashc_w2i[m.group(1)]+
			self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+
			self.hashm_w2i[u"aOY"]+self.hashmd_w2i[m.group(4)], my_string)
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
        		([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])OY''', lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+
    			self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+self.hashm_w2i[u"aOY"], my_string)

	my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([AiIuUeEoO])([MHz])''', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+
			self.hashm_w2i[m.group(4)]+self.hashmd_w2i[m.group(5)], my_string)
	my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
        		([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([AiIuUeEoO])''', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+
			self.hashm_w2i[m.group(4)], my_string)
        if a:
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
        		([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])a([MHz])''', lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+
    			self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+self.hashmd_w2i[m.group(4)], my_string)
	    my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])a''', lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+
    			self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)], my_string)

	my_string = re.sub(u'''([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])
    			([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])''', lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+
    			self.hashc_w2i[m.group(2)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(3)]+self.hashc_w2i[u"_"], my_string)
        if eV:
	   my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])eV([MHz])', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[u"eV"]+self.hashmd_w2i[m.group(3)], my_string)
	   my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])eV', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[u"eV"], my_string)
        if EY:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])EY([MHz])', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+self.hashm_w2i[u"EY"]+
    			self.hashmd_w2i[m.group(3)], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])EY', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[u"EY"], my_string)
        if oV:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])oV([MHz])', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[u"oV"]+self.hashmd_w2i[m.group(3)], my_string)
	    my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])oV', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[u"oV"], my_string)
        if OY:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])OY([MHz])',
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[u"OY"]+self.hashmd_w2i[m.group(3)], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])OY', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[u"OY"], my_string)

        my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([AiIuUeEoO])([MHz])', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[m.group(3)]+self.hashmd_w2i[m.group(4)], my_string)
        my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([AiIuUeEoO])',
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashm_w2i[m.group(3)], my_string)
        if a:
	    my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])a([MHz])', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)]+
			self.hashmd_w2i[m.group(3)], my_string)
	    my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])a', 
    			lambda m: self.hashc_w2i[m.group(1)]+self.hashc_w2i[u"_"]+self.hashc_w2i[m.group(2)], my_string)
        if eV:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])eV([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
    			self.hashm_w2i[u"eV"]+self.hashmd_w2i[m.group(2)], my_string)
	    my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])eV', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashm_w2i[u"eV"], my_string)
        if EY:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])EY([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
        		self.hashm_w2i[u"EY"]+self.hashmd_w2i[m.group(2)], my_string)
	    my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])EY', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashm_w2i[u"EY"], my_string)
        if oV:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])oV([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
     			self.hashm_w2i[u"oV"]+self.hashmd_w2i[m.group(2)], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])oV', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashm_w2i[u"oV"], my_string)
        if OY:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])OY([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
    			self.hashm_w2i[u"OY"]+self.hashmd_w2i[m.group(2)], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])OY', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashm_w2i[u"OY"], my_string)

        my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])Z([AiIuUeEoO])([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
    			self.hashc_w2i[u"Z"]+self.hashm_w2i[m.group(2)]+self.hashmd_w2i[m.group(3)], my_string)
        if Z:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])Z([AiIuUeEoO])', lambda m: self.hashc_w2i[m.group(1)]+
    			self.hashc_w2i[u"Z"]+self.hashm_w2i[m.group(2)], my_string)
	    my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])Za([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
        		self.hashc_w2i[u"Z"]+self.hashmd_w2i[m.group(2)], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])Za', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashc_w2i[u"Z"], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])Z', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashc_w2i[u"Z"]+self.hashc_w2i[u"_"], my_string)
        if q:
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])q([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashm_w2i[u"q"]+self.hashmd_w2i[m.group(2)], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])q', lambda m: self.hashc_w2i[m.group(1)]+
			self.hashm_w2i[u"q"], my_string)
            #Added for the case Vowel(U090B)+Modifier
            my_string = re.sub(u'(aq)([MHz])', lambda m: self.hashv_w2i[m.group(1)]+self.hashmd_w2i[m.group(2)], my_string)
        ...
        my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([AiIuUeEoO])([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
    		    self.hashm_w2i[m.group(2)]+self.hashmd_w2i[m.group(3)], my_string)
        my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])([AiIuUeEoO])',lambda m: self.hashc_w2i[m.group(1)]+
    		    self.hashm_w2i[m.group(2)], my_string)
        if a:
	    my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])a([MHz])', lambda m: self.hashc_w2i[m.group(1)]+
    			self.hashmd_w2i[m.group(2)], my_string)
            my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])a', lambda m: self.hashc_w2i[m.group(1)], my_string)

        my_string = re.sub(u'([kKgGfcCjJFtTdDNwWxXnpPbBmyrlvSsRh])', lambda m: self.hashc_w2i[m.group(1)]+
		    self.hashc_w2i[u"_"], my_string)
    ...
    def iscii2unicode_hin(self, iscii):
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hashh_i2u.get(m.group(1), u""), iscii)
        return unicode_
    
    def iscii2unicode_tel(self, iscii):
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hasht_i2u.get(m.group(1), u""), iscii)
        return unicode_
    
    def iscii2unicode_pan(self, iscii):
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hashp_i2u.get(m.group(1), u""), iscii)
        return unicode_
    
    def iscii2unicode_kan(self, iscii):
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hashk_i2u.get(m.group(1), u""), iscii)
        return unicode_
    
    def iscii2unicode_mal(self, iscii):
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hashm_i2u.get(m.group(1), u""), iscii)
        return unicode_
    
    def iscii2unicode_ben(self, iscii): 
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hashb_i2u.get(m.group(1), u""), iscii)
        return unicode_
    
    def iscii2unicode_tam(self, iscii):
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hashcta_i2u.get(m.group(1), u""), iscii)
        return unicode_
    
    def iscii2unicode_ori(self, iscii):
        unicode_ = re.sub(u'([\xA1-\xFA])', lambda m: self.hasho_i2u.get(m.group(1), u""), iscii)
        return unicode_
	...
	"""Convert ISCII to WX"""
        # CONSONANT+HALANT
        my_string = re.sub(u"([\xB3-\xD8])\xE8",lambda m:self.hashc_i2w[m.group(1)], my_string)
        # CONSONANT+NUKTA+MATRA+MODIFIER 
        my_string = re.sub(u"([\xB3-\xD8])(\xE9)([\xDA-\xE7])([\xA1-\xA3])",lambda m:self.hashc_i2w[m.group(1)]+
    			self.hashc_i2w[m.group(2)]+self.hashm_i2w[m.group(3)]+self.hashmd_i2w[m.group(4)], my_string)
        # CONSONANT+NUKTA+MATRA 
        my_string = re.sub(u"([\xB3-\xD8])(\xE9)([\xDA-\xE7])",lambda m:self.hashc_i2w[m.group(1)]+
    			self.hashc_i2w[m.group(2)]+self.hashm_i2w[m.group(3)], my_string)
        # CONSONANT+NUKTA+MODIFIER 
        my_string = re.sub(u"([\xB3-\xD8])(\xE9)([\xA1-\xA3])",lambda m:self.hashc_i2w[m.group(1)]+
    			self.hashc_i2w[m.group(2)]+self.hashmd_i2w[m.group(3)], my_string)
        # CONSONANT+NUKTA+HALANT (added -Rashid 29-December-11 wanaKZvaxara)
        my_string = re.sub(u"([\xB3-\xD8])(\xE9)(\xE8)",lambda m:self.hashc_i2w[m.group(1)]+self.hashc_i2w[m.group(2)], my_string)
        # CONSONANT+NUKTA 
        my_string = re.sub(u"([\xB3-\xD8])(\xE9)",lambda m:self.hashc_i2w[m.group(1)]+self.hashc_i2w[m.group(2)]+u"a", my_string)
        # CONSONANT+MATRA+MODIFIER 
        my_string = re.sub(u"([\xB3-\xD8])([\xDA-\xE7])([\xA1-\xA3])",lambda m:self.hashc_i2w[m.group(1)]+
    			self.hashm_i2w[m.group(2)]+self.hashmd_i2w[m.group(3)], my_string)
        # CONSONANT+MATRA 
        my_string = re.sub(u"([\xB3-\xD8])([\xDA-\xE7])",lambda m:self.hashc_i2w[m.group(1)]+self.hashm_i2w[m.group(2)], my_string)
        #CONSONANT+MODIFIER 
        my_string = re.sub(u"([\xB3-\xD8])([\xA1-\xA3])",lambda m:self.hashc_i2w[m.group(1)]+u"a"+self.hashmd_i2w[m.group(2)], my_string)
        #CONSONANT 
        my_string = re.sub(u"([\xB3-\xD8])",lambda m:self.hashc_i2w[m.group(1)]+u"a", my_string)
        #VOWEL+MODIFIER, VOWEL 
        my_string = re.sub(u"([\xA5-\xB2])([\xA1-\xA3])",lambda m:self.hashv_i2w[m.group(1)]+self.hashmd_i2w[m.group(2)], my_string)
        my_string = re.sub(u"[\xA4]([\xA1-\xA3])", lambda m: u"a"+self.hashmd_i2w[m.group(1)], my_string)
        my_string = re.sub(u"([\xA5-\xB2])",lambda m:self.hashv_i2w[m.group(1)], my_string)
        ...
        my_string = re.sub(u"([\xF1-\xFA])",lambda m:self.digits_i2w[m.group(1)], my_string)
        return my_string
    				
    def unicode2iscii_hin(self, unicode_):
        # Normalize Unicode values (NUKTA variations)
        iscii_hin = re.sub(u"([\u0958-\u095F])", lambda m:self.unicode_norm_hashh_u2i.get(m.group(1), u"")+u"\u093C", unicode_)
        # Convert Unicode values to ISCII values
        iscii_hin = re.sub(u"([\u0900-\u097F])", lambda m:self.hashh_u2i.get(m.group(1), u""), iscii_hin)
        return iscii_hin
    
    def unicode2iscii_tel(self, unicode_):
        # Convert Telugu Unicode values to ISCII values
        iscii_tel = re.sub(u"([\u0C01-\u0C6F])", lambda m:self.hasht_u2i.get(m.group(1), u""), unicode_)
        return iscii_tel
    
    def unicode2iscii_pan(self, unicode_):
        # Normalize Unicode values (NUKTA variations)
        iscii_pan = re.sub(u"([\u0A59-\u0A5B\u0A5E])", lambda m:self.unicode_norm_hashp_u2i.get(m.group(1), u""), unicode_)
        ...
        iscii_pan = re.sub(u"([\u0A01-\u0A75])", lambda m:self.hashp_u2i.get(m.group(1), u""), iscii_pan)
        return iscii_pan
    
    def unicode2iscii_kan(self, unicode_):
        # Normalize Unicode values (NUKTA variations), needs to be modified yet, PAWAN
        iscii_kan = re.sub(u"([\u0958-\u095F])", lambda m:self.unicode_norm_hashk_u2i.get(m.group(1), u""), unicode_)
        # Convert Unicode values to ISCII values
        iscii_kan = re.sub(u"([\u0C80-\u0CFF])", lambda m:self.hashk_u2i.get(m.group(1), u""), iscii_kan)
        return iscii_kan
    
    def unicode2iscii_mal(self, unicode_):
        # Convert Unicode values to ISCII values
        iscii_mal = re.sub(u"([\u0D00-\u0D6F])", lambda m:self.hashm_u2i.get(m.group(1), u""), unicode_)
        return iscii_mal
    
    def unicode2iscii_ben(self, unicode_):
        # Convert Unicode values to ISCII values
        iscii_ben = re.sub(u"([\u0980-\u09EF])", lambda m:self.hashb_u2i.get(m.group(1), u""), unicode_)
        return iscii_ben
    ...
    def unicode2iscii_tam(self, unicode_):
        ...
        iscii_tam = re.sub(u"([\u0B82-\u0BEF])", lambda m:self.hashta_u2i.get(m.group(1), u"") , iscii_tam)
        return iscii_tam
    
    def unicode2iscii_ori(self, unicode_):
    	# Normalize Unicode values (NUKTA variations)
    	# Convert Unicode values to ISCII values
        iscii_ori = re.sub(u"([\u0B00-\u0B7F])",lambda m:self.hasho_u2i.get(m.group(1), u""), unicode_)
        return iscii_ori
    ...
    def findlang(self, my_string):
	"""Get language code of the string"""
	if not isinstance(my_string, unicode):
	    my_string = my_string.decode('utf-8')
	
        val = ord(my_string[0])
        if val >= 2304 and val <= 2431:
	    return "hin"
        elif val >= 2432 and val <= 2559:
            return "ben"
        elif val >= 2560 and val <= 2659:
	    return "pan"
        elif val >= 2944 and val <= 3071:
            return "tam"
        elif val >= 3072 and val <= 3199:
	    return "tel"
        elif val >= 3200 and val <= 3327:
            return "kan"
        elif val >= 3328 and val <= 3455:
            return "mal"
        elif val >= 1536 and val <= 1791:
            return "urd"
        else: 
            return "eng"
